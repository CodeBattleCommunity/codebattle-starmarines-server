...
THREE.OrbitControlsMod = function ( object, domElement, centers ) {
    ...
    this.update = function () {
        ...
        if ( lastPosition.distanceTo( this.object.position ) > 0 || lastPosition.distanceTo( this.object.position ) < 3000 ) {
...
var shaderBlock = {
    ...
    loadSun : function (size) {
        ...
        //        console.log(sun);
        renderBlock.sun = sun;
	    renderBlock.sun = new THREE.Object3D();
        return sun;
...
var renderBlock = {
    ...
    createTextTexture: function (planetId) {
      ...
      context.shadowOffsetX = 1;
        ...
        context.font = renderType == 'WebGL' ? "bold 42pt sans-serif" : "bold 26pt Arial";
        ...
        context.fillText(renderBlock.planets[planetId].unitsCount, canvas.height / 3,  2*canvas.height / 6, canvas.width);

        context.font =  renderType == 'WebGL' ? "32pt Arial": "20pt Arial";
        context.fillText(owner , canvas.height / 3,  3.0*canvas.height / 6,canvas.width);
...
//        var texture = new THREE.Texture(renderBlock.generateSpriteTexture(256));
    ...
    initRenderer: function () {
        ...
        if(renderType=='WebGL'){
	        ...
	        renderer.setClearColorHex(0x040404, 0);
...
//        renderBlock.scene.add(particles);
    ...
    programStats: function( context , size) {
        ...
        for(player in players ){
            heightSize = renderType == 'WebGL' ? 26 : 60;
            name = players[player].name;
            x = 10;
            y = i*heightSize *1.4;
            width = stats[name] / (max + 1); //TODO  normalization
            //                        context.rotate(Math.PI*2/(i*modelBlock.players.length));
            fontSize = heightSize;
            font = renderBlock.textFont;
...
//	        console.log(context.measureText(name));
    ...
    createStats: function(){
        ...
        if(renderType == 'Canvas' ) {
            ...
            stats.scale = new THREE.Vector3(0.5,0.5,0.5);
...
//	        renderBlock.camera = new THREE.OrthographicCamera( renderBlock.SCREEN_WIDTH / - 1, renderBlock.SCREEN_WIDTH / 1, renderBlock.SCREEN_HEIGHT / 1, renderBlock.SCREEN_HEIGHT / - 1, 0.1, 10000);
        } else {

            renderBlock.camera = new THREE.PerspectiveCamera(cameraConfig.fov, renderBlock.SCREEN_WIDTH /  renderBlock.SCREEN_HEIGHT, cameraConfig.near, cameraConfig.far);
//	        renderBlock.camera = new THREE.OrthographicCamera( renderBlock.SCREEN_WIDTH / - 1, renderBlock.SCREEN_WIDTH / 1, renderBlock.SCREEN_HEIGHT / 1, renderBlock.SCREEN_HEIGHT / - 1, 0.1, 10000);
	        renderBlock.camera.position.set(0, 200, 2500);
//	        renderBlock.camera.scale.set(1.1, 1.1, 1);
...
//            renderBlock.camera.position.set(0, 200, 9000);
//	        renderBlock.camera.scale.set(1.1, 1.1, 1);
    ...
    programStroke: function ( context ) {
        ...
        //        context.lineWidth = 0.1;
        //        context.beginPath();
        //        context.arc( 0, 0, 0.7, 0, Math.PI * 2, true );
        //        context.closePath();
        //        context.stroke();
        //        context.lineWidth = 0.1;
        //        context.beginPath();
...
//	                blending: THREE.SubtractiveBlending
                ...
                sprite.scale.x = sprite.scale.y = 1.3/planetMesh.scale.x  ;
...
//                sun = new THREE.Mesh(new THREE.SphereGeometry(renderBlock.getPlanetSize(planet.type), lod, lod / 2), planetMaterial);
	            sun = renderBlock.drawRing(renderBlock.getPlanetSize(planet.type),0xffffff);
                sun.rotation.x = Math.PI/180*90;
...
//                var planetMesh = new THREE.Mesh(new THREE.SphereGeometry(renderBlock.getPlanetSize(planet.type), lod, lod / 2),planetMaterial );
	            var planetMesh = renderBlock.drawRing(renderBlock.getPlanetSize(planet.type),0xffffff);
                renderBlock.fillPlanet(planetMesh, planet);
...
//            renderBlock.sun.rotation.x -= 0.005;
    ...
    generateCountTexture : function (count, color, size, _canvas) {
        ...
        context.font = " 14pt " + renderBlock.textFont;
...
//	    console.log(1);
        ...
        for(var e = 0; e < booms.length; e++) {
            if(renderType=='Canvas'){
                booms[e].scale.x = booms[e].scale.y +=  0.3*(e + scale/30) * delta * (1/planetSize) ;
            } else {
    ...
    arrowBuilder: function (from, to, up) { //add cases for boundary values
        ...
        phi = Math.tan(nullPoint.y, nullPoint.x);
	    console.log(122);
        ...
        var spline2D = new THREE.SplineCurve3([
            ...
            mid,
            arrow.to]);
        spline2D = new THREE.LineCurve(arrow.from, arrow.to);
        ///////////////////////////////
        ...
        line2 = new THREE.Line(geoLine2, new THREE.LineBasicMaterial({
            color: 0x996644
        }));
...
//                    var material = new THREE.Sprite({
                ...
                sunAtmos.visible = true;
	            sunAtmos = new THREE.Object3D();
                temp.add(sunAtmos);
            //                temp.add();
            }
        ...
        for(var e = minWaves; e < maxWaves; e++) {
            if(renderType == 'Canvas') {
                ...
                var texture = renderBlock.generateSpriteTexture(800);
...
//	                mergeWith3D:true
	            ...
	            boom = renderBlock.drawRing(100, 0x444444);
...
//	    window.setTimeout(renderBlock.onWindowResize, 100);
//	    renderBlock.renderer.setSize(renderBlock.SCREEN_WIDTH, renderBlock.SCREEN_HEIGHT);
        initBlock.actionUpdater = window.setInterval(loopBlock.updateActions, renderBlock.turnSpeed/2);
//	    window.setTimeout(renderBlock.onWindowResize,renderBlock.turnSpeed/2);
    },
...
//	if(Stats !== undefined) {var stats = new Stats();
